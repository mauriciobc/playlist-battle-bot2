/**
 * Run the event-driven driver against a mock Mastodon started by
 * bot-against-mock.mts, reading the endpoint it wrote.
 *
 * The driver reads ./.env from its working directory, and
 * test/integration/.env holds real credentials, so it is never modified.
 * Instead the driver runs from a temp directory holding a rewritten .env
 * pointed at the mock.
 *
...
/**
 * Thirty-two YouTube ids, dealt 16 to the host and 16 to the player.
 *
 ...
 *
 * A few of these ids have since gone private or been removed upstream, so
 * the pool is deliberately oversized: a dead link is rejected by the bot
 * ("Não foi possível resolver esse vídeo" / "Could not resolve that
 * video."), which the submit step reports, and the remaining links still
 * fill both playlists. That is also why the submit step's pass condition is
 * the poll, not the absence of rejections.
 */
const TUNES = [
...
async function main(): Promise<void> {
  ...
  const endpoint = readEnvFile(endpointFile);
  ...
  // The third voter gets its own account, so the tally stays readable in
  // the bot's database; its origin defaults to PLAYER1_API_URL.
  vals.VOTER1_TOKEN = endpoint.MOCK_VOTER_TOKEN ?? "voter-token";

  // Split TUNES in two non-overlapping lists (see TUNES): the driver refuses
  // a shared video outright, since hasRoundCollision would auto-tie the round.
  ...
  const child = spawn("npx", ["tsx", join(HERE, "driver.ts")], {
    cwd: stage,
