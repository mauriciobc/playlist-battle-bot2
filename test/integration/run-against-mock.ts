...
async function main(): Promise<void> {
  const server = new MockMastodonServer({
    ...
    // Bind every interface: the bot usually runs in Docker, where 127.0.0.1
    // is the container's own loopback and cannot reach a process on the host.
    host: "0.0.0.0",
  });
  await server.start();
  const port = new URL(server.baseUrl).port;
  // The driver stays in this process on loopback; the bot gets a
  // host-reachable origin.
  const botUrl = process.env.MOCK_ADVERTISE_URL ?? `http://127.0.0.1:${port}`;
  console.log(`mock Mastodon listening on ${server.baseUrl}`);
  console.log(`  BOT_BASE_URL for the bot: ${botUrl}`);

  // Stage a working copy of the WHOLE integration directory, not just
  // driver.ts: the driver imports ./mastodon-helpers.js and ./driver-exit.js,
  // so copying the file alone left those dangling. cpSync of the directory
  // keeps the relative graph intact.
  const stage = mkdtempSync(join(tmpdir(), "pbb-mock-"));
  cpSync(HERE, stage, {
    recursive: true,
    filter: (src) => !src.endsWith("/.env") && !src.endsWith("node_modules"),
  });
  // The staged copy sits outside the repo, so rewrite the ../../src imports
  // into absolute ones that still resolve.
  const driverStage = join(stage, "driver.ts");
  const code = readFileSync(driverStage, "utf-8").replace(
    /from "\.\.\/\.\.\/src\//g,
